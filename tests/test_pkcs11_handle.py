...
class TestPKCS11Handle(unittest.TestCase):
    ...
    def test_import_keypair_ed25519(self) -> None:
        ...
        asyncio.run(PKCS11Session.create_keypair("bb", key_type="ed25519"))
        pk_info, identifier = asyncio.run(PKCS11Session.public_key_data("bb", "ed25519"))
        ...
        with self.assertRaises(ValueError):
            asyncio.run(
                PKCS11Session.sign(imported_key_label, b"data_to_be_signed", mechanism=Mechanism.SHA256_RSA_PKCS)
            )
            pk_info, identifier = asyncio.run(PKCS11Session.public_key_data(imported_key_label))
    ...
    def test_sign_and_verify_data_ed25519(self) -> None:
        """
